...
public class MistIdleManager {

    private static final String TAG = "MistIdleManager";

    private static final String ACTION_SCAN = "com.android.systemui.mist.ACTION_IDLE_SCAN";
    private static final String ACTION_HALT = "com.android.systemui.mist.ACTION_IDLE_HALT";
    ...
    private static volatile MistIdleManager sInstance;
    ...
    private MistIdleManager(@NonNull Context context) {
        ...
        mIoExecutor = Executors.newSingleThreadExecutor(r -> {
            Thread t = new Thread(r, "MistIdleManager-IO");
            t.setDaemon(true);
            return t;
        });
    ...
    public static void initManager(@NonNull Context context) {
        if (sInstance == null) {
            synchronized (sLock) {
                if (sInstance == null) {
                    sInstance = new MistIdleManager(context);
                }
            }
        }
    }
    ...
    @Nullable
    public static MistIdleManager getInstance() { return sInstance; }
    ...
    public void executeManager() {
        ...
        if (!mEnabled) {
            Log.d(TAG, "MistIdleManager disabled — skipping");
            return;
        }
    ...
    public void haltManager() {
        ...
        Log.d(TAG, "Halting MistIdleManager");
        cancelCallbacks();
        mIsRunning = false;
    }
    ...
    public void cleanup() {
        ...
        Log.d(TAG, "MistIdleManager cleaned up");
    }
    ...
    private void performIdleScan() {
        ...
        for (Map.Entry<String, AppConfig> entry : mAppConfigCache.entrySet()) {
            ...
            if (MistIdleConstants.PROTECTED_PACKAGES.contains(pkg)) {
